...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Evaluate the response using the rubric below. Return only a JSON "
            "object mapping every criterion to a score from 0 to 1.\n\n"
            f"Question:\n{question}\n\n"
            f"Response:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for result in scores_batch:
            ...
            if mean_score is not None and position in (1, "first"):
                first_scores.append(mean_score)
            elif mean_score is not None and position in (2, "second"):
                second_scores.append(mean_score)
            ...
            if (
                ...
                second_scores.append(float(second))
        ...
        if first_scores and second_scores:
            ...
            positional_bias = first_average - second_average > 0.05
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review the failure trace and apply the root-cause fix"
            )
            rows.append(
                f"| F{index:03d} | {safe_cell(failure.failure_type or 'unknown')} | "
                f"{safe_cell(self.find_root_cause(failure))} | "
                f"{safe_cell(suggestion)} | Open |"
            )
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        normalized_types = {
            (failure.failure_type or "unknown").casefold() for failure in failures
        }
        suggestions: list[str] = []
        if "hallucination" in normalized_types:
            suggestions.append(
                "Implement a grounding check that rejects claims unsupported by retrieved evidence"
            )
        if "irrelevant" in normalized_types or "off_topic" in normalized_types:
            suggestions.append(
                "Add intent-focused prompt examples and route ambiguous questions for clarification"
            )
        if "incomplete" in normalized_types or "low_completeness" in normalized_types:
            suggestions.append(
                "Improve retrieval coverage and require answers to include all conditions and exceptions"
            )
        if "refusal" in normalized_types:
            suggestions.append(
                "Calibrate safety guardrails with allowed-domain examples to reduce unnecessary refusals"
            )

        general_suggestions = [
            "Inspect low-scoring retrieval traces and tune chunking, top-k, and reranking",
            "Add representative failure cases to the golden dataset as regression tests",
            "Calibrate evaluation thresholds with human-reviewed domain labels",
        ]
